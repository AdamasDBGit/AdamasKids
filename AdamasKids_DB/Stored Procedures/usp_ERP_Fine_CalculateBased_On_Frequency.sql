...
--    @s_StudentID VARCHAR(10)='24-0042'
	...
	Declare @FrequencyID int,@FrequencyCode Varchar(50),@FineTagID int,@waiveoff bit
	,@InvoiceHeaderID bigint

	Select 
	 @FineTagID= IVP.I_FineTagID
	,@waiveoff=IVP.Is_Fine_waiveroff
	,@InvoiceHeaderID=IVP.I_Invoice_Header_ID
	from T_Invoice_Parent IVP With (Nolock)
	Inner Join T_Student_Detail SD With (Nolock) on SD.I_Student_Detail_ID=IVP.I_Student_Detail_ID
	Inner Join T_Enquiry_Regn_Detail RD With (Nolock) on RD.I_Enquiry_Regn_ID=SD.I_Enquiry_Regn_ID
	Inner Join T_Brand_Center_Details BCD on BCD.I_Centre_Id=RD.I_Centre_Id
	where SD.S_Student_ID=@s_StudentID and BCD.I_Brand_ID=@BrandID



	--------------------------------------------------------------
	Select @FrequencyID= FreqType,@FrequencyCode=PT.S_Installment_Frequency 
	...
	If (@FineTagID is Not Null or @FineTagID<>0)  
			...
			Inner Join T_Invoice_Child_Detail ICD 
			on ICH.I_Invoice_Child_Header_ID=ICD.I_Invoice_Child_Header_ID
			Where ICH.I_Invoice_Header_ID=@InvoiceHeaderID and ICD.S_Invoice_Number Like '%TEMP%'
			Group By ICH.I_Invoice_Header_ID,
			ICD.Dt_Installment_Date,ICD.I_Installment_No


		--Select * from @InvDetails

	Declare @ID Int =1,@lst Int,@InstallmentDt Date,@FineAmount Numeric(18,2)
	SET @lst=(select Count(id) from @InvDetails)
	While @ID<=@lst
	Begin
    ...
    Begin
	SET @InstallmentDt =(select convert(date,Dt_Installment_Date) from @InvDetails where ID=@id)
	...
	SET @FineAmount=(Select Top 1 N_Fine_Amount from T_ERP_Fee_Fine_Details where I_Fee_Fine_H_ID=@FineTagID
	and @LateDays Between I_Frm_Range and I_To_Range)
	--Select @LateDays,@FineAmount
	Update @InvDetails set N_FineAmount=Isnull(@FineAmount,0) where ID=@ID
    END
	End
    ELSE IF @FrequencyCode = 'Monthly'
    BEGIN
	   Declare @FinalMonthly_Amount numeric(18,2)
       SET @FineAmount=(Select Top 1 N_Fine_Amount from T_ERP_Fee_Fine_Details 
	   where I_Fee_Fine_H_ID=@FineTagID)
	   Declare @MonthDiff int 
       SET @MonthDiff=(SELECT DATEDIFF(month, @InstallmentDt,@PaymentDate))
	   If @MonthDiff<>0
	   Begin
	   ...
	   End 
	   	Update @InvDetails set N_FineAmount=Isnull(@FinalMonthly_Amount,0) where ID=@ID
   END
...
END
 SELECT T_Invoice_Child_Header,Dt_Installment_Date,InstallmentNo,Isnull(N_FineAmount,0) as FineAmount from  @InvDetails --------Select Final Fine data
...
Else
Begin
SELECT T_Invoice_Child_Header,Dt_Installment_Date,InstallmentNo,Isnull(N_FineAmount,0) from  @InvDetails
End
End