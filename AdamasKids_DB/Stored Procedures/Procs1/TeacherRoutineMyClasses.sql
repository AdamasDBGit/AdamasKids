--exec [TeacherRoutineMyClasses] 1065,'2024-06-28'  
...
BEGIN      
 ...
 WHERE TETTP.I_Student_Class_Routine_ID = TESCR.I_Student_Class_Routine_ID and SSP.I_Month_No=MONTH(@Day) and (@D IS NULL or SSP.I_Day_No =@D)      
    ) > 0 THEN 1 ELSE 0 END AS IsLogbook,     
 TFM.S_Faculty_Name TeacherName,      
 TESCRW.S_ClassWork ClassWork      
 ,ttc.TotalStud as Total_Student    
 ,ttc.I_Stream_ID I_Stream_ID    
...
COALESCE(TST.I_Stream_ID, 0),TST.I_Stream_ID     
) as ttc on ttc.I_School_Session_ID= TERSH.I_School_Session_ID    
and ttc.I_Class_ID=tc.I_Class_ID 
and ttc.I_Stream_ID=STT.I_Stream_ID    Or   STT.I_Stream_ID is null
 WHERE       
