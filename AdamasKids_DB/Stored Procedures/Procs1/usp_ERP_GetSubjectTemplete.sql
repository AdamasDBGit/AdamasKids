...
BEGIN  
 ...
 select STH.I_Brand_ID as BrandID,  
  ...
  CASE              WHEN EXISTS (SELECT 1 FROM [T_ERP_Subject_Structure_Header] WHERE I_Subject_Template_Header_ID = ISNULL(@iSubjectTemplateHeaderID, STH.I_Subject_Template_Header_ID))             THEN 1              ELSE 0          END AS SubjectMapped 
 
  from [T_ERP_Subject_Template_Header] as STH  
