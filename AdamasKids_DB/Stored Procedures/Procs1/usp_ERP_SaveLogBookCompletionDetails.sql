	...
	begin
		...
		where SP.I_Subject_Structure_Plan_ID=@iLogbookID
			...
			begin
				...
				IF NOT EXISTS
				(
				select * from T_ERP_Subject_Structure_Plan_Execution_Remarks where I_Subject_Structure_Plan_ID=@iLogbookID
				)
				...
				begin
					...
					select @iLogbookID,@iCompletionPercentage,@sRemarks,@UserID,GETDATE(),GETDATE() 
