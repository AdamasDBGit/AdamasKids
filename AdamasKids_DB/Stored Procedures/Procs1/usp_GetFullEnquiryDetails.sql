...
BEGIN  
 ...
 SET @studentID =(Select I_Student_Detail_ID from T_Student_Detail	where I_Enquiry_Regn_ID=@iEnquiryRegnID)
 SET NOCOUNT ON;  
 DECLARE @FatherFullName NVARCHAR(300) = (select S_Father_Name from  [dbo].[T_Enquiry_Regn_Detail] where I_Enquiry_Regn_ID = @iEnquiryRegnID);  
  ...
  WHERE TEERD.I_Enquiry_Regn_ID = @iEnquiryRegnID;  
   ...
   --TEERD.S_First_Name AS S_Father_First_Name,       --TEERD.S_Middile_Name AS S_Father_Middile_Name,    
 ...
 ISNULL(sgc.I_School_Group_ID,TEERD.I_School_Group_ID) I_School_Group_ID,
  --EERD.I_Class_ID AS I_Course_Applied_For,  
