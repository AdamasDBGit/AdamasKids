...
BEGIN
    ...
    WHERE IP.I_Centre_Id IN (SELECT I_Center_ID FROM @TempCenter)
        ...
        AND CONCAT(LTRIM(RTRIM(SD.S_First_Name)), ' ', ISNULL(LTRIM(RTRIM(SD.S_Middle_Name)) + ' ', ''), LTRIM(RTRIM(SD.S_Last_Name))) LIKE ISNULL(@FullName, CONCAT(LTRIM(RTRIM(SD.S_First_Name)), ' ', ISNULL(LTRIM(RTRIM(SD.S_Middle_Name)) + ' ', ''), LTRIM(RTRIM(SD.S_Last_Name)))) + '%'
